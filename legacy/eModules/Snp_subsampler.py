...
def random_subsample_df(df, target_rows, seed):
    if not isinstance(target_rows, int) or target_rows <= 0:
        raise ValueError('Target number for subsampling must be a positive integer.')
...
def random_subsample_dfs(df1, df2, target_rows, seed):
    ...
    df1_exclusive = df1.loc[~df1.index.isin(inner_df.index)]
    df2_exclusive = df2.loc[~df2.index.isin(inner_df.index)]

    total_rows = len(df1) + len(df2) - len(inner_df)
    target_inner_rows = int(target_rows * (len(inner_df) / total_rows))
    target_df1_rows = int(target_rows * (len(df1_exclusive) / total_rows))
    target_df2_rows = target_rows - target_inner_rows - target_df1_rows

    subsample_inner_df = inner_df.sample(n=target_inner_rows, random_state=seed)
    subsample_df1 = df1_exclusive.sample(n=target_df1_rows, random_state=seed)
    subsample_df2 = df2_exclusive.sample(n=target_df2_rows, random_state=seed)
...
if __name__ == '__main__':
    ...
    usage = '''python Snp_subsampler.py -s 123 -r 20000 -o *.txt --df1 snp_df1 --df2 snp_df2 (optional)'''
