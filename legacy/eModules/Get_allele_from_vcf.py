...
def replace_dna_sequence(template, pos, ref, allele):
    if len(ref) == 1:
        template[pos] = allele
    elif len(ref) > 1:
        start = pos
...
def get_allele_sequences(bed_obj, fasta_obj, bed_vcf):
    ...
    # store sample's variants in this interval
    sample_variants = {}
    ...
    sample_sequences = {}
    for sample in sample_list:
        sample_sequences[sample] = {0: list(ref_sequence), 1: list(ref_sequence)}

    for record in vcf_reader.fetch(bed_chrom, bed_start, bed_end + 1):
        # to 0-based
        pos_0 = record.pos - 1
        # relative position in bed
        pos_r = pos_0 - bed_start

        ref = record.ref
        alts = record.alts
        alleles = list(ref) + list(alts)

        for sample in record.samples:
            GT1 = record.samples[sample]['GT'][0]
            GT2 = record.samples[sample]['GT'][1]
            if GT1 and GT2:
                sample_variants[sample][pos_r] = {'ref': ref, 0: alleles[GT1], 1: alleles[GT2]}

    for sample, variants in sample_variants.items():
        for pos, variant in variants.items():
            # for haplotype 0
            if variant['ref'] != variant[0]:
                replace_dna_sequence(sample_sequences[sample][0], pos, variant['ref'], variant[0])
            # for haplotype 1
            if variant['ref'] != variant[1]:
                replace_dna_sequence(sample_sequences[sample][1], pos, variant['ref'], variant[1])
