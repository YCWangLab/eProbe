...
def cal_dimer_score_seq(seq, k, kmer_freq_dict, probe_num):
    ...
    return (dimer_score / probe_num) * 100


def cal_dimer_score_fasta(fasta_file, k, kmer_freq_dict, probe_num):
    dimer_score = {}
    sequences = list(SeqIO.parse(fasta_file, 'fasta'))
    for record in sequences:
        id = record.id
        score = cal_dimer_score_seq(str(record.seq), k, kmer_freq_dict, probe_num)
        dimer_score[id] = score
    return dimer_score


def process_probe_file(probe_file, k, kmer_freq_dict, probe_num):
    dimer_score = cal_dimer_score_fasta(probe_file, k, kmer_freq_dict, probe_num)
    result_list = []
...
if __name__ == '__main__':
    ...
    probe_num = len(read_fasta(args.fasta))
    ...
    with multiprocessing.Pool(processes=int(args.thread)) as pool:
        for split_file in split_files:
            results.append(
                pool.apply_async(process_probe_file, args=(split_file, int(args.kmer), filtered_kmer_dict, probe_num,)))
        for result in results:
    ...
    print('Calculating dimer score completed.', datetime.datetime.now())

    if os.path.exists(args.output + '.dimer_eProbe_temp'):
        shutil.rmtree(args.output + '.dimer_eProbe_temp')
