...
# function: call kraken2 to filter background noise
...
if __name__ == '__main__':
    ...
    usage = """python Background_probe_filter.py -f probe.fasta -d kraken2_databse -t 10 --min_hits 1 --confidence 0.15 -o background_filtered_probe.fasta"""
    ...
    parser.add_argument("-d", "--db", dest="db", action="store", nargs='?',
                        help="Kraken2 database(s) (separated with comma).", metavar="STRING", required=True)
    parser.add_argument("-t", "--thread", dest="thread", action="store", nargs='?', default=1,
                        help="Number of thread (default: 1).", metavar="STRING")
    parser.add_argument("--min_hits", dest="min_hits", action="store", nargs='?', default=2,
                        help="Minimum number of non-overlapping kmer hits (separate kmer stacks) to filter out (default: 2).",
                        metavar="STRING")
    parser.add_argument("--confidence", dest="confidence", action="store", nargs='?', default=0,
                        help="Minimum percentage of kmers out of a sequence needed to be assigned to a node (default: 0).",
                        metavar="STRING")
    ...
    if len(args.db.split(",")) == 1:
        ...
        temp_output = f"{args.output}.background_filtered_tempout"
        for db in args.db.split(","):
            run_kraken2(temp_input, db, int(args.thread), args.min_hits, args.confidence,
                        temp_output)
            temp_input = f"{args.output}.background_filtered_tempin"
    ...
    print("Filtered out %s probes from %s probes in background noise filtering." % (after_filtering, before_filtering),
