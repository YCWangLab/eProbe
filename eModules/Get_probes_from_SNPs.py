...
def get_probes_with_df(snp_df, method, fa_dict, left_flank=0, right_flank=0, shift=0):
    ...
    for row in snp_df.itertuples():
        ...
        if method == "pos":
            ...
            # Ensure the start is not less than 0
            ref_start = max(0, start)
            # Ensure the end does not exceed the chromosome length
            ref_end = min(end, len(fa_dict[chromosome]) - 1)
            header = f"{chromosome}:{ref_start + 1}-{ref_end + 1}_{row.pos}_{row.type}_{row.ref}_{row.alt}"
        elif method == "edge":
            # 1-based to 0-based
        ...
        sequence = fa_dict[chromosome][ref_start:ref_end + 1]
        probe_dict.setdefault(chromosome, {})[header] = sequence.upper()

    return probe_dict


def replace_TargetSNPs(probe_dict):
...
if __name__ == '__main__':
    ...
    usage = """python Get_probes_from_SNPs.py -d SNPs_df.txt -g genome.fasta -l 81 -s shift -o output"""
    ...
    parser.add_argument("-s", "--shift", dest="shift", action="store", nargs='?', default="0",
                        ...
                        metavar="STRING")
    parser.add_argument("-m", "--method", choices=['pos', 'edge'], dest="method", action="store", nargs='?',
                        default="pos",
    ...
    SNPs_df = pd.read_csv(args.snp_df, delimiter='\t', header=0)
    ...
    if args.replace == "on":
        ...
        probes_dict = replace_TargetSNPs(probes_dict)
    if args.type:
        print(f"Select {args.type} SNPs ...", datetime.datetime.now())
        probes_dict = filter_type(probes_dict)
    print("Outputting probes ...", datetime.datetime.now())
