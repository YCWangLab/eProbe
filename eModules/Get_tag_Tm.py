...
if __name__ == '__main__':
    ...
    parser.add_argument("-m", "--method", dest="method", action="store", nargs='?', default="Tm_NN",
                        help="Method of calculating melting temperature (Tm_NN/Tm_GC; default: Tm_NN).", choices=["Tm_NN", "Tm_GC"],
                        metavar="STRING")
    ...
    with multiprocessing.Pool(processes=int(args.thread)) as pool:
        ...
        for header, seq in probes.items():
            results.append(pool.apply_async(process_probe, (header, seq, Tm_cal), kwds={"method": args.method}))
